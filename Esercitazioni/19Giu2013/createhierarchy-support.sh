...
inv=true #variabile usata per determinare la validitÃ  dell'input dell'utente (true => input non valido)
...
do echo "\nInserire la lista dei file:"
   ...
   if test "$flist"
   then case $flist in
        */*) echo "Il nome di un file non puÃ² contenere il carattere '/'!";; 
        *)   for f in $flist 
             do > $f
	     done
	     inv=false;;
...
do echo "\nInserire la lista delle sottodirectory:"
   ...
   if test "$dlist"
   then case $dlist in
        */*) echo "Il nome di una directory non puÃ² contenere il carattere '/'!";; 
        *)   for d in $dlist 
