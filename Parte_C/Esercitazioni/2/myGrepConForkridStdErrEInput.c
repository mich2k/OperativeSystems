...
int main(int argc, char ** argv) {
  ...
  if (pid == 0) {
    ...
    /* ridirezionamo lo standard output su /dev/null perche' ci interessa solo se il comando grep ha
successo o meno */
    close(1);
    close(2);   // chiudo lo stderr
    close(0);
...
STANDARD OUTPUT E’ RIDIRETTO SU /dev/null */
    exit(-1);
    /* torniamo al padre un -1 che sara' interpretato come 255 e quindi identificato come
errore */
  }
