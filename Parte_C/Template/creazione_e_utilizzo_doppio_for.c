...
int main(int argc, char** argv) {
    ...
    N = atoi(argv[1]);
    if (N <= 0 || N >= 255) {
        printf("Errore: il param non è strettamente positivo o non e' strettamente minore di 255: %d\n", N);
        exit(2);
    }
    ...
    for (i = 0; i < N; i++) {
        if ((pid = fork()) < 0) {
           ...
           INDISPENSABILE che ci sia la presenza di una exit per fare in modo
           che il processo figlio termini e NON esegua il ciclo for, che invece
           deve essere eseguito solo dal padre */
        }
    } /* fine for */
    /* codice del padre */
